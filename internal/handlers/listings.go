...
import (
	...
	"github.com/azulgautam79/olx-api/internal/httpx"
...
func (lh ListingHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	row := lh.db.QueryRowContext(ctx, `
		...
		($1, $2, $3, $4) RETURNING id, title, description, price, city, created_at`, &req.Title, &req.Description, &req.Price, &req.City)

	var out CreateListingResponse
	if err := row.Scan(&out.ID, &out.Title, &out.Description, &out.Price, &out.City, &out.CreatedAt); err != nil {
